...
{"7. Incorporate web search information to make the content richer, more accurate, and more in-depth, including real data and cases" if web_context else ""}"""
    ...
    def _call_llm(self, prompt: str, task_id: str = "") -> str:
        ...
        payload = {
            ...
            "temperature": 0.7,
            "max_tokens": 4096,
            "stream": not self.config["is_local"],
        }
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not payload.get("stream", False):
                    return self._call_llm_sync(headers, payload)
                return self._call_llm_stream(headers, payload, task_id)
    ...
    def _call_llm_sync(self, headers: dict, payload: dict) -> str:
        """非流式调用LLM API"""
    ...
    def _call_llm_stream(self, headers: dict, payload: dict, task_id: str = "") -> str:
        """流式调用LLM API"""
        from .progress import progress_tracker
    ...
    def _check_response_status(response):
        ...
        if response.status_code == 404:
            logger.error("LLM API端点不存在(404)：请检查端点地址和模型名称")
            raise ValueError("API端点或模型不存在，请检查端点地址和模型名称是否正确")
